...
| NFR-S4 | `API_CONDITION`의 `url`은 저장만 하더라도 **SSRF 위험**이 있으므로, 실행 Phase(No.26)에서 사설 IP 대역(`127.0.0.0/8`, `10/8`, `172.16/12`, `192.168/16`, `169.254/16`) 차단이 필요함을 스키마 주석과 설계서에 명시한다. 이번 Phase에는 저장 시 형식 검증까지만 한다. |
| NFR-S5 | `API_CONDITION.headers`에 담기는 인증 토큰은 평문 저장 대상이다. 이번 Phase는 **저장 자체를 허용하되** 화면에서 마스킹 표시하고, 암호화 저장은 No.26/No.45(데이터 거버넌스) 과제로 기록한다. |
...
| 챗봇 간 대화 자산 **복사/이관 UI** | 이번엔 CSV 내보내기/가져오기로 우회(FR-6-30). 전용 마이그레이션 기능은 후속 |
