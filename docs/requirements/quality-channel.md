...
- **No.24 하이브리드 CS**(실시간 모니터링·상담원 개입·응답힌트), **No.42 옴니채널 통합 인박스**: 상담원 관점 기능 전체 제외. 단 이들이 도입되면 DD-18(세션 비영속) 결정을 재검토해야 함을 기록한다. **[이행 2026-09-25 No.24] 상담원 전환은 상담 전용 짧은 폴링으로 도입됐다(WebSocket/SSE 없음) — DD-18은 봇 대화 상태 기준으로 유지하고 개입 세션에만 상담 스레드를 둔다(`hybrid-cs-설계.md` · ADR-0036).** **[이행 2026-09-26 No.42] 통합 인박스도 DD-18을 봇 대화 상태 기준으로 유지하고, 식별된 세션과 볼 일이 생긴 세션만 고객 연결 행을 둔다(`omnichannel-inbox-설계.md` · ADR-0042).**
...
| 항목 | 제외 사유 / 이관 대상 |
|---|---|
| 대화 상태의 **서버 영속화**(세션 테이블/Redis)와 만료 정리 배치 | DD-18 유지. 상담원 인계(No.24)·옴니채널(No.42) 도입 시 재검토 **[No.24 재검토 완료 2026-09-25 — 봇 대화 상태는 봉투 유지, 개입 세션만 상담 스레드(ADR-0009 부분 갱신)]** **[No.42 재검토 완료 2026-09-26 — 봇 대화 상태는 봉투 유지, 고객·연결·스레드·기록만 서버(ADR-0009 부분 갱신 · ADR-0042)]** |
