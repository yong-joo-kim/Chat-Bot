...
> ※ 번호 충돌 안내: 위 표의 `X-5`(본 문서 고유 번호, `deploy-schedule-sealing.spec.ts` 확인 항목)와 `environment-sealing.spec.ts`의 **E-5 시험 자체**는 서로 다르다. E-5(`packages/dialogue-engine` 봉인 표면 검사, FR-0-149)는 2026-09-26 No.41 후속 작업에서 git 작업 트리 의존(`git status --porcelain`) 방식을 골든 스냅샷 비교 방식으로 재설계했다 — 그 변경 이력의 X 번호(`X-5`)와 편차 기록(`I-3`)은 `docs/02-spec/workflow-automation-설계.md` §21.3·§27에 있다(본 문서의 X 번호 체계와는 무관·재사용 없음).
