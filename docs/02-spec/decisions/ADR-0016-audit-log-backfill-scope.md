...
- 이번 Phase에 **`ApiCallLog`를 만들지 않는다.** **[이행 2026-09-24 No.26 — 원문 없는 메타데이터 전용으로 도입. 문서 끝 갱신 참고]** 기록 대상인 No.26(레거시 API 연동)이 미구현이고 `API_CONDITION` 아웃풋은 실행조차 되지 않는다(ADR-0008). **쓰기 주체 없는 스키마 금지**(ADR-0004).
