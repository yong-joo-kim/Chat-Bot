...
## 2026-09-26 — 611f050 (선행 분리 커밋 99f8de0·9ba78cb·1c2543b)

feat: 옴니채널 통합 인박스(No.42) 기능그룹 구현

- 채널 = 웹 실연동 + 수동 기록 채널(전화·이메일·방문) + 시뮬레이션 채널(시험 고객 전용). 수동 기록·시뮬레이션은 대화 로그·통계·학습 큐에 쓰지 않는다(격리, O-10)
- 식별 = 고객사가 서명한 로그인 토큰(JWS Compact HS256, `x-cb-identity` 헤더) — `sub`/`iat`/`exp` 필수, 시계 오차 ±300초, 최대 수명 24시간. 해시만 저장(원 회원 번호·토큰·전체 sessionId 컬럼 0), 검증은 챗봇별 `OMNI_IDENTITY_SECRET__<REF>` 리졸버 1파일에서만
- 고객·연결·병합: 고객 1명 = 스레드 1개, 연결은 `(chatbotId, sessionId)` 전체 유일(재지정 + 이전 고객 기록), 되돌릴 수 있는 병합(서로 다른 로그인 회원끼리는 병합 금지), 익명 세션에 유효 토큰이 오면 익명 → 식별 고객 자동 승격 병합. `IDENTITY` 연결 분리는 ADMIN 전용(토큰 재생 대응)
- 전역 통합 인박스: 스레드 상태(열림/보류/종료)·수동 배정+가져가기·ADMIN 관리 전역 태그·메모·타임라인(연결된 대화·상담을 한 화면에), 여러 챗봇 통합 뷰 + 기존 상담 콘솔은 그대로 유지(고객 카드 링크로 연계)
- 위젯 `identify()`/`data-identity-token` 속성으로 서명 토큰을 헤더에 실어 보낸다(토큰이 없으면 기존과 바이트 동일, gzip 12.89KB)
- 거버넌스 편입: 마스킹 → 암호화(`EncryptedFieldId` +2) → 보존 소거(`RetentionTargetKind` +2, 전역만) · `VIEW` 열람 감사 화면 3개 추가(8→11)
- `@Public()` 8개 그대로 유지, `dialogue-engine` 변경 0, 신규 권한 0종(챗봇별 참여 설정은 기존 `security:write`/`chatbot:write` 재사용)
- No.24 하이브리드 CS 상담 이력에 `sessionRef` 추가(콘솔 연계), 기존 응답은 불변

### PM 결정 요점(P-1~P-12, 전부 추천안 채택) + 추가 확정

P-1(b) 채널 = 웹 실연동 + 수동 기록(전화·이메일·방문) + 시뮬레이션(시험 고객 전용), 시뮬레이션·수동 기록은 로그·통계·학습 큐에 쓰지 않는다 · P-2(a) 식별 = 고객사 서명 로그인 토큰(JWT HS256)·기존 메시지 요청 헤더·해시만 저장·상담원 수동 연결/분리·서명 키는 챗봇별 env 참조 · P-3(b) 새 전역 통합 인박스·고객 1명 = 스레드 1개·목록엔 볼 일이 생긴 고객만·여러 챗봇 통합 뷰·기존 상담 콘솔은 그대로 + 고객 카드 링크 · P-4(a) 컨텍스트 = 상담원 화면(이력+사실만 담은 고객 카드+메모)·봇 동작 불변 · P-5 해시+암호화된 표시 이름·보존기간 경과 시 파기 · P-6 연결 행 방식·되돌릴 수 있는 병합·서로 다른 로그인 회원끼리 병합 금지 · P-7 상태 열림/보류(해제 시각)/종료·수동 배정+가져가기·ADMIN 관리 전역 태그 · P-8 신규 권한 0 · P-9 챗봇별 참여 설정·기본 꺼짐 · P-10 마스킹→암호화→보존 소거·VIEW 감사 화면 3개 추가 · P-11 2차 = 요구사항 §9 목록 · P-12 GPU 1(하향)·구축형 ○·구독형 ○.

추가 확정: `IDENTITY` 연결 분리는 ADMIN 전용(일반 상담원 불가, 토큰 재생 대응) · 익명 세션에 유효 식별 토큰이 도착하면 익명 고객을 식별 고객으로 자동 승격 병합한다(되돌리기는 ADMIN) · 고객 키 계산에 쓰는 `OMNI_CUSTOMER_KEY_SECRET`은 운영 중 교체 금지(교체하면 기존 고객과의 통합이 끊긴다 — K-3 2차로 탐지 로직 유보).

### 마이그레이션

`20260926210000_omnichannel_inbox` 1개 — 전부 `CREATE`(신규 테이블 `customers`·`customer_links`·`inbox_threads`·`inbox_thread_tags`·`inbox_tags`·`inbox_entries`·`chatbot_inbox_settings`, `Chatbot` 역참조는 컬럼 0). 기존 테이블 재정의·`ALTER TABLE`·백필 0건.

### 배포 절차

① `prisma migrate deploy`(테이블 신설뿐 — API 중지 불필요) → ② API 배포(위젯 배포 순서 무관 — 위젯은 토큰이 없으면 헤더를 보내지 않고, 구버전 API는 헤더를 무시한다) → ③ 콘솔 배포 → ④ (선택) 인박스를 쓰려면 환경변수 확인: `OMNI_INBOX_ENABLED`(기본 `true` — 끄면 인박스·설정 API 404, 식별 검증 0, 저장된 데이터는 유지), `OMNI_CUSTOMER_KEY_SECRET`(≥32바이트, 없거나 짧으면 식별 기능만 꺼짐 — **운영 중 교체 금지**), 챗봇별 `OMNI_IDENTITY_SECRET__<REF>`(+`__PREV`는 호스트 비밀 교체 중 병행 검증용, 선택), `OMNI_MERGE_REVERT_HOURS`(기본 24) 등 나머지는 전부 기본값으로 동작 → ⑤ 챗봇 상세 > 통합 인박스 설정에서 참여를 켠다(기본 꺼짐, FR-0-183) → ⑥ 고객사 임베드 스니펫에 `data-identity-token` 속성(또는 `identify()` 호출)을 설정해 로그인 토큰을 위젯에 전달한다 → ⑦ 거버넌스 모드(No.45)를 켠 설치는 기존 보존 정책 화면에 `INBOX_TEXT`/`CUSTOMER_IDENTITY` 2종이 자동으로 추가된다(전역만, 선택 — 생략 시 현재값 유지).

### API 변경 공지

- `GET /inbox/identity-spaces` 응답 형태가 `string[]`에서 `{ ref, chatbots: {id,name}[] }[]`로 바뀌었다(하위 비호환) — 콘솔이 "이 공간을 쓰는 챗봇" 목록을 표시하기 위함이며, 관리자 콘솔 외의 소비자는 없다.
- No.24 하이브리드 CS 상담 이력(`GET .../handoff-history`)에 `sessionRef` 필드가 추가됐다(추가뿐, 하위 호환).
- 보존 정책 API(`GlobalRetentionUpdateSchema`·`RetentionPreviewRequestSchema`·`GovernanceMapResponse`)의 `kinds`가 6종에서 8종으로 늘었다(신규 2종은 선택 키 — 생략하면 기존 6키 요청도 그대로 통과).

### 알려진 한계

- 외부 채널(카카오톡 등) 실연동은 2차 범위다 — 이번 그룹은 웹 실연동 + 수동 기록 + 시뮬레이션까지만 구현했다.
- 유효기간(`exp`) 내 식별 토큰의 재사용(재생)은 탐지하지 않는다(서명·시계 오차·만료만 검증).
- **I-1**: 목록 쿼리(`GET /inbox/threads`) 예산을 §9.1 설계값(논리 7단계)에서 실측 기준(서비스 단독 ≤10 · HTTP 종단 ≤12)으로 재조정했다 — SQLite는 Prisma `relationJoins`를 지원하지 않아 관계 로딩마다 물리 쿼리가 추가로 붙는다(Postgres 전환 시 재검토 대상).
- 식별·신호(상담 시작·턴 기록) 처리가 인스턴스당 단일 pending 체인으로 전역 직렬화된다 — 트래픽이 몰리면 인박스 반영 지연이 커질 수 있다(HTTP 응답 자체는 지연되지 않음). 챗봇당 체인 분리 등 병렬화는 이번 그룹의 범위가 아니다 — 부하 시험으로 실제 병목 여부를 확인해야 한다.
- O-18(`=== 'WEB'` 리터럴 비교 0건 정적 검사)은 이번 그룹이 실제로 정리한 범위(`live-sessions.service.ts`, C-4)로 좁혔다 — `channels.service.ts`·`workflow-trigger.service.ts`의 기존 비교문은 손대지 않았다(전수 적용은 후속 과제).
- 다음 회차 권고 시험 8건(상세 엔드포인트 전용 쿼리 수 계측, 부하 시험 등 — code-reviewer R1 기록 I-5 참고).
- 통합 통계 시험의 간헐 ECONNRESET(본 그룹 범위 밖, 전체 스위트 병렬 실행 시에만 드물게 재현)과 hybrid-cs 통합 시험의 간헐 레이스(단독 실행 9/9 통과) — 둘 다 기존 결함이다.
