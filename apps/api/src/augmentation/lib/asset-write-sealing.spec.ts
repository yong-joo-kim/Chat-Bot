...
describe('제안 ≠ 자산 구조적 봉인 정적 검사 — ADR-0025 §13, AC-L4-3', () => {
  ...
  describe('S-1: Intent/Keyword 자산 쓰기 Prisma 호출은 소유 서비스 파일 밖에서 0건이다', () => {
    ...
    // [신규 No.22] `createMany`를 포함한다 — 분리 적재기(`asset-transfer.loader.ts`)의 생성 전용 쓰기도
    // 이 검사 대상이다(topic-system-설계.md §25 패치 B-6).
    const writeCallPattern = /(?:prisma|tx)\.(intent|keyword)\.(create|update|updateMany|upsert|createMany)\(/;
    ...
    // "승인 없는 자산 주입" 경로가 아니다.
    // [신규 No.22] `topics/topic-assignment.service.ts`(자산 6종 topicId 일괄 쓰기 유일 파일, §17 T-7)와
    // `asset-transfer/asset-transfer.loader.ts`(분리 적재기, 생성 전용 · §17 T-5)가 4·5번째로 추가된다 —
    // 둘 다 "승인 없는 자산 내용 변경"이 아니라 분류(topicId)/생성 전용 경로다.
    it.each(apiFileContents.filter(({ content }) => writeCallPattern.test(content)).map(({ f }) => f))(
      '%s는 intents.service.ts · keywords.service.ts · versions/restore/version-restore.applier.ts · topics/topic-assignment.service.ts · asset-transfer/asset-transfer.loader.ts 중 하나여야 한다',
      (file) => {
        ...
        expect(
          normalized.endsWith('intents/intents.service.ts') ||
            ...
            normalized.endsWith('topics/topic-assignment.service.ts') ||
            normalized.endsWith('asset-transfer/asset-transfer.loader.ts'),
        ).toBe(true);
      },
    );

    it('실제로 걸리는 파일이 정확히 5개다(가드 — 검사 자체가 무력화되지 않았음을 확인)', () => {
      ...
      expect(offenders.filter((f) => f.endsWith('topics/topic-assignment.service.ts'))).toHaveLength(1);
      expect(offenders.filter((f) => f.endsWith('asset-transfer/asset-transfer.loader.ts'))).toHaveLength(1);
