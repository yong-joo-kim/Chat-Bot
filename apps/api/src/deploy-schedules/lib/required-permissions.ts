 ...
 */
export function requiredPermissions(action: DeployScheduleAction, params: { versionId?: string; enableWebChannel?: boolean; enabled?: boolean }): Permission[] {
  switch (action) {
...
/** G3 허용 동작(§11) — `RESTORE_VERSION`·`PUBLISH`만. `SET_WEB_CHANNEL`은 응답 내용이 바뀌지 않는다. */
export const POST_RUN_TEST_APPLICABLE_ACTIONS: readonly DeployScheduleAction[] = ['RESTORE_VERSION', 'PUBLISH'];
