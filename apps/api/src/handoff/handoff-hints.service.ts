...
import type { HintAnswerItem, HintCannedItem, HintResponse } from '@chat-bot/shared-types';
...
export class HandoffHintsService {
  ...
  async getHints(chatbotId: string, sessionRef: string): Promise<HintResponse> {
    ...
    const value = await this.compute(chatbotId, source);
  ...
  private async compute(chatbotId: string, source: { key: string; text: string }): Promise<HintResponse> {
    ...
    if (settings.semanticEnabled) {
      const semantic = await this.semanticMatch.score(chatbotId, source.text, bundle, {
