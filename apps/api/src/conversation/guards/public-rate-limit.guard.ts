 ...
 *
 * [K-1, §2.6·§7.3] `@PublicRateBucket()`이 붙은 핸들러(폴링 전용)는 위 두 버킷을 **소비하지 않고**
 * `poll-ip`(공용 600/분) + 경로별 키 버킷만 소비한다 — 같은 NAT 뒤 폴링이 일반 전송을 429로 만들던
 * 결함을 해소한다(보류 답변 폴링·상담 폴링 공용).
 */
@Injectable()
export class PublicRateLimitGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (bucketSpec) {
      this.consumePollBuckets(req, res, now, bucketSpec);
      return true;
    }
  ...
  /** `poll-ip`(1축, 공용) + 경로 키(2축) — 기존 `ip`/`session` 버킷은 전혀 건드리지 않는다. */
  private consumePollBuckets(req: Request, res: Response, now: number, spec: PublicRateBucketSpec): void {
    const trustProxy = this.config.get<boolean>('TRUST_PROXY') ?? false;
    const pollIpLimit = this.config.get<number>('PUBLIC_POLL_RATE_LIMIT_IP_PER_MIN') ?? 600;

    const ip = resolveClientIp(req, trustProxy);
    const ipResult = this.store.consume(`poll-ip:${ip}`, now, pollIpLimit, WINDOW_MS);
    if (!ipResult.allowed) {
      res.setHeader('Retry-After', String(ipResult.retryAfterSec));
      throw new ApiException('RATE_LIMITED', 429, '요청이 많습니다. 잠시 후 다시 시도해 주세요.');
    }
    ...
    const keyResult = this.store.consume(`poll-key:${spec.key.ns}:${resolvedKey}`, now, perKeyLimit, WINDOW_MS);
